...
FFEMFXMeshSection* UFEMMesh::CreateMeshSectionFromFEMFile(AMD::FmTetMeshBuffer* tetMeshBuffer, AMD::FmBvh* BVH, int32 sectionIdx)
{
	...
	for (int i = 0; i < (normalsBuffer.Num() / 3); i++)
	{
		...
		FVector converted(-vec.z, vec.x, vec.y);
	...
	for (int i = 0; i < (tangentsBuffer.Num() / 3); i++)
	{
		...
		FVector converted(ConvertFEMFXVectorToUnreal(importTangent));
	...
	{
		UV0.Add(FVector2D(uvsBuffer[(i * 3)], uvsBuffer[(i * 3) + 1]));
	}
	...
	{
		FVector inPos(positionBuffer[i * 3], positionBuffer[(i * 3) + 1], positionBuffer[(i * 3) + 2]);
...
FFEMFXMeshSection* UFEMMesh::CreateMeshSection(UStaticMesh* StaticMesh, AMD::FmTetMeshBuffer* tetMeshBuffer, AMD::FmBvh* BvHierarchy, int sectionIdx)
{
	...
	if (posVertBuffer)
	{
		...
		{
			// VERTEX POSITIONS
			FVector vertexPosition = posVertBuffer->VertexPosition(VertIdx);
			...
			// VERTEX NORMALS
			FVector Normal = meshVertBuffer->VertexTangentZ(VertIdx);
			...
			//VERTEX TANGENTS
			FVector VertexTan = meshVertBuffer->VertexTangentX(VertIdx);
			...
			//UV0
			FVector2D UV = meshVertBuffer->GetVertexUV(VertIdx, 0);
			UV0.Add(UV);
...
FBoxSphereBounds UFEMMesh::UpdateBounds()
{
	...
	{
		FVector vertexPosition = ImportedResource->FEMFXMeshSections[0].VertexBuffer[i].Position;
		vertexPosition *= 100;
...
FFEMFXMeshSection* UFEMMesh::CreateMeshSection_LinearColor(int32 SectionIndex, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals, const TArray<FVector2D>& UV0, const TArray<FLinearColor>& VertexColors, const TArray<FFEMFXMeshTangent>& Tangents,
...
FFEMFXMeshSection* UFEMMesh::CreateMeshSection(int32 SectionIndex, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals, const TArray<FVector2D>& UV0, const TArray<FColor>& VertexColors, const TArray<FFEMFXMeshTangent>& Tangents,
